...
고용24 OPEN-API는 가장 먼저 확보할 핵심 키지만, 그것 하나만 있으면 끝나는 구조는 아니다. 서울특별시교육청 하이잡과 지역 교육청 취업지원센터처럼 별도 키 없이 공식 공개 페이지에서 확인 가능한 교육청 소스는 시스템이 전용 어댑터로 점검하되, 직접 결과 카드가 아니라 1차 공식 공고를 보강하는 보조검증 출처로만 사용한다.

로컬에서 먼저 시험할 때는 저장소 루트의 `.env.example`을 보고 같은 이름을 `.env.local`에 적는다. 실제 배포 자동화는 아래 순서대로 GitHub Actions Secret에 저장해야 한다.
...
| --- | --- | --- | --- |
| 1 | 인사혁신처 공공취업정보 | `MPM_PUBLIC_JOB_SERVICE_KEY` 또는 `DATA_GO_KR_SERVICE_KEY` | 발급받은 공공데이터포털 인증키를 Secret 저장 |
| 2 | 기재부 공공기관 채용정보 | `MOEF_PUBLIC_RECRUIT_SERVICE_KEY` 또는 `DATA_GO_KR_SERVICE_KEY` | 발급받은 공공데이터포털 인증키를 Secret 저장 |
