...
create table if not exists public.review_change_history (
  ...
  change_type text not null check (change_type in ('annotation_saved', 'annotation_deleted', 'progress_saved', 'report_generated', 'report_submitted', 'report_delivered')),
  target_id text,
  before_payload jsonb,
  after_payload jsonb,
  occurred_at timestamptz not null default now()
);
create index if not exists review_change_history_assignment_idx on public.review_change_history (assignment_id, occurred_at desc);
create index if not exists review_change_history_reviewer_idx on public.review_change_history (reviewer_user_id, occurred_at desc);
