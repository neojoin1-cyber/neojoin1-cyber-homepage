...
function renderSubmissionIntegrity(integrity) {
  ...
  $("#integrity-details").innerHTML = `${perDocumentMarkup ? `<section><h3>자료별 미확인 범위</h3>${perDocumentMarkup}</section>` : ""}${uncheckedMarkup ? `<section><h3>미확인 위치 상세</h3>${uncheckedMarkup}${uncheckedOmitted ? `<p class="integrity-legacy">나머지 ${uncheckedOmitted}건도 위 자료별 집계에 포함되어 있습니다. 각 자료의 첫 미확인 위치에서 이어서 확인할 수 있습니다.</p>` : ""}</section>` : ""}${suspiciousMarkup ? `<section><h3>확인 속도 주의 위치</h3>${suspiciousMarkup}${suspiciousOmitted ? `<p class="integrity-legacy">나머지 주의기록 ${suspiciousOmitted}건도 전체 집계와 대표 보고서에 포함됩니다.</p>` : ""}</section>` : ""}${integrity.unknownTimingCount ? `<section><h3>시간 판정 불가 기록</h3><p class="integrity-legacy">속도기록 기능 적용 전 또는 화면 노출 기록 없이 확인된 문단 ${Number(integrity.unknownTimingCount)}개입니다. 부정 판정은 하지 않으며 대표 보고서에는 판정 불가로 명시됩니다.</p></section>` : ""}${!issues ? '<div class="integrity-clear">미확인 또는 초고속·일괄 확인 주의기록이 없습니다.</div>' : ""}`;
