...
function hasCollegeOnlyApplicantSignal(value) {
  ...
  return (COLLEGE_APPLICANT_PATTERN.test(text) && !COLLEGE_APPLICANT_DISQUALIFIED_PATTERN.test(text))
    || (DEGREE_ONLY_APPLICANT_PATTERN.test(text) && !DEGREE_PREFERENCE_PATTERN.test(text));
}
...
function classifyProcess(raw) {
  ...
  if (criticalProtectedRecruit) {
    processTrack = 'exam-formal';
    writtenExam = hasExam ? 'confirmed' : 'likely';
    confidence = 'high';
    labels.push('핵심 공채 보호');
    note = '현재 진행 중 핵심 감시 공채로 지정된 공고라 혼합 전형 문구가 있어도 공채 상세 정보에서 누락되지 않도록 보호했습니다.';
  } else if (studentRecommendBlocked) {
...
function isUnsuitableForHighSchoolChannel(item) {
  const text = eligibilityText(item);
  if (criticalCurrentPriority(item) < 99) return false;
