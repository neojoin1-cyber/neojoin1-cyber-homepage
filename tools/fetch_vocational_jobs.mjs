...
const COLLECTION_AUDIT_FILE = path.join(OUTPUT_DIR, 'job-collection-audit.json');
...
const QUALIFICATION_REVIEW = new Map();
...
const FINANCE_LARGE_COMPANY_OFFICIAL_WATCHLIST = [
  ...
  { employer: 'SK하이닉스', group: 'large-company', url: 'https://talent.skhynix.com/hub/ko/apply/job', tags: ['대기업', '반도체'] },
  ...
  { employer: 'NH농협은행', group: 'finance', url: 'https://with.nonghyup.com/jbnf/jbnfLst.do?srcAuthDsc=1', fallbackUrls: ['https://with.nonghyup.com/index.jsp', 'https://with.nonghyup.com/'], timeoutMs: 24000, tags: ['1금융권', '은행'] },
...
const REGIONAL_EDUCATION_OFFICIAL_WATCHLIST = [
  ...
  { employer: '세종특별자치시교육청 취업지원센터', group: 'education-office', url: 'https://www.sje.go.kr/job/na/ntt/selectNttList.do?bbsId=1422&mi=50461', timeoutMs: 24000, tags: ['교육청', '직업계고', '취업지원'] },
...
function genericRecordToRaw(record, source, publicSourceUrl, feedEntry = {}) {
  ...
  return {
    ...
    sourceId: pickRecordField(record, ['sourceId', 'id', 'idx', 'seq', 'sn', 'no', '공고번호', '채용공시ID', 'recruitId', 'recrutPblntSn', 'recrutPbancNo', 'pbancNo']) || sha([source.id, title, company, detailUrl, feedEntry.url].join('|')),
    title,
    company,
...
function parseGenericOfficialFeed(body, source, sourceUrl, feedEntry = {}) {
  ...
  if (trimmed.startsWith('<')) {
    const xmlRecords = collectXmlRecords(trimmed, publicSourceUrl).map((record) => ({
      ...
      source: source.id,
      sourceName: source.name,
      sourceId: record.sourceId || sha([source.id, record.title, record.company || feedEntry.employer, record.url].join('|')),
...
function assessRecruitRoles(raw = {}, verifiedText = '') {
  ...
  const qualificationAssessment = assessStudentEligibility({ ...raw, recruitField: roles.join(','), qualification: raw.qualification || verifiedText });
  if (qualificationAssessment.roleEvidence.length) {
    const entries = qualificationAssessment.roleEvidence;
    const excludedByRole = (role) => ROLE_LEVEL_EXCLUSION_PATTERN.test(role) || hasStudentUnsuitableProfessionalRole(role);
    const eligibleRoles = qualificationAssessment.status === 'eligible' ? qualificationAssessment.eligibleRoles.filter((role) => !excludedByRole(role)) : [];
    return { version: 2, mixed: entries.length >= 2, sourceText,
      eligibleRoles, reviewRoles: compactTags(entries.filter((x) => x.status !== 'eligible' && !excludedByRole(x.role)).map((x) => x.role)),
      excludedRoles: compactTags([...roles, ...entries.map((x) => x.role)].filter(excludedByRole)),
      hasEligibleRole: eligibleRoles.length > 0, needsRoleReview: !eligibleRoles.length };
  }
  const eligibleRoles = [];
  const reviewRoles = [];
  const excludedRoles = [];
  ...
  for (const role of roles) {
    ...
    } else if (qualificationAssessment.status === 'eligible'
      && (qualificationAssessment.eligibleRoles.includes(role) || !qualificationAssessment.roleEvidence.length)
      && overallStudentAccess && STUDENT_ACCESSIBLE_ROLE_PATTERN.test(role)) {
...
function buildStudentChannelAssessment(raw, process) {
  const qualificationAssessment = assessStudentEligibility(raw);
  const text = normalizeSpace([
    raw.title,
    raw.company,
    raw.region,
  ...
  const hardBlocked = hasCanceledRecruitSignal(text)
    || qualificationAssessment.status !== 'eligible'
    || collegeOnly
    || professionalOnly
  ...
  reasons.push(...qualificationAssessment.reasons);
...
function officialSourceBriefingStatus(items) {
  return {
    enabled: true,
    required: true,
    engine: 'official-source-rules',
    generatedAt: CHECKED_AT,
    attempted: items.length,
    succeeded: items.filter((item) => item.teacherBriefing?.teacherShareText).length,
    failed: items.filter((item) => !item.teacherBriefing?.teacherShareText).length,
    status: 'ok'
  };
}

function removeLegacyAiBriefing(item = {}) {
  const briefing = item.teacherBriefing;
  if (!briefing) return item;
  const cleanLines = (values) => (Array.isArray(values) ? values.filter((value) => !/Ollama/i.test(String(value))) : values);
  const cleaned = {
    ...briefing,
    generator: 'official-source-briefing-v1',
    sourceLines: cleanLines(briefing.sourceLines),
    summaryLines: cleanLines(briefing.summaryLines),
    schoolActionItems: cleanLines(briefing.schoolActionItems),
    studentActionItems: cleanLines(briefing.studentActionItems),
    schoolCheckSections: Array.isArray(briefing.schoolCheckSections)
      ? briefing.schoolCheckSections.filter((section) => !/Ollama/i.test(`${section?.title || ''} ${section?.text || ''}`))
      : briefing.schoolCheckSections,
    teacherShareText: String(briefing.teacherShareText || '').split(/\n\[Ollama 보강\]\n?/i)[0].trim()
  };
  delete cleaned.ollamaAssist;
  item.teacherBriefing = cleaned;
  return item;
}
...
function normalizeItem(raw) {
  ...
  COLLECTION_ASSESSED.set(`${item.source}:${item.sourceId || item.id}`, item);
  const qualificationAssessment = studentChannelAssessment.qualificationAssessment;
  if (qualificationAssessment.status !== 'eligible') {
    QUALIFICATION_REVIEW.set(`${item.source}:${item.sourceId || item.id}`, {
      ...
      status: qualificationAssessment.status, reasons: qualificationAssessment.reasons,
      evidence: qualificationAssessment.evidence, roleEvidence: qualificationAssessment.roleEvidence,
      attachments: raw.qualificationAttachments || []
    });
  } else {
    QUALIFICATION_REVIEW.delete(`${item.source}:${item.sourceId || item.id}`);
  }
  return item;
}
...
function studentRecruitPriority(item = {}) {
  ...
  if (qualificationAssessment.status !== 'eligible' || assessment.hardBlocked) {
    tier = 10;
    label = '지원 자격 검토 대상';
...
function validateStudentPriorityFixtures() {
  ...
  const graduateCandidate = studentRecruitPriority({
    ...
    studentChannelAssessment: {
      qualificationAssessment: { status: 'eligible', explicitHighSchool: true },
...
function publicationBlockReason(item = {}) {
  ...
  if (assessStudentEligibility(item).status !== 'eligible') return 'student-qualification-unverified';
...
function normalizePublicationItem(item = {}) {
  let next = { ...item };
  const repairs = [];
  next.studentChannelAssessment = buildStudentChannelAssessment({ ...next,
    qualification: next.qualificationText, description: next.qualificationText || ''
...
function applyPublicationSafetyGuards(items = [], archiveItems = [], supplementalItems = []) {
  ...
  const addStudentRecruitReview = (item, listName, reason = '') => {
    ...
    if (report.studentRecruitReviewSamples.length < 2000) {
...
function buildFeedHealth(payload, safetyReport = null, statusOverride = '') {
  ...
  return {
    ...
    summary: {
      ...
      automatedBriefingReady: summary.automatedBriefingReady || 0,
      automatedBriefingAttempted: summary.automatedBriefingAttempted || 0,
      automatedBriefingFailed: summary.automatedBriefingFailed || 0
    },
...
async function readPreviousItems() {
  try {
    ...
    const lists = [
      ...
      Array.isArray(payload.supplementalItems) ? payload.supplementalItems : [],
...
export function moefRecordToRaw(record, source = catalogSource('moef-public-recruit')) {
  ...
  return { ...raw, sourceId: String(record.recrutPblntSn),
    ...
    qualificationEvidenceIncomplete: !record.aplyQlfcCn,
    recruitField: record.ncsCdNmLst || '', recruitNumber: String(record.recrutNope || ''),
    processText: htmlText(record.scrnprcdrMthdExpln || ''),
    description: htmlText([record.aplyQlfcCn, record.scrnprcdrMthdExpln].filter(Boolean).join('\n')),
    url: `https://job.alio.go.kr/recruitview.do?idx=${encodeURIComponent(record.recrutPblntSn)}`,
    attachments: (record.files || []).map((entry) => entry.file || entry)
      .map((file) => ({ title: file.atchFileNm || '공식 첨부문서', url: cleanUrl(file.url) })).filter((x) => x.url)
  };
}
...
async function fetchMoefPublicRecruit() {
  ...
  const inventory = await collectPages({
    pageSize: 100, maxPages: 40, recordKey: (row) => row.recrutPblntSn,
    fetchPage: async (pageNo, numOfRows) => {
      const body = await fetchWithTimeout(buildPublicDataUrl(MOEF_PUBLIC_RECRUIT_LIST_URL, key,
        { pageNo, numOfRows, resultType: 'json', ongoingYn: 'Y' }), { timeoutMs: PUBLIC_API_TIMEOUT_MS });
      const response = JSON.parse(body);
      if (![0, 200].includes(Number(response.resultCode))) throw new Error('MOEF response code');
      const records = Array.isArray(response.result) ? response.result.map((x) => x.item || x) : [];
      if (!Array.isArray(response.result) && Number(response.totalCount) > 0) throw new Error('MOEF missing records');
      return { records, totalCount: response.totalCount };
    }
  });
  const rawItems = inventory.records.map((record) => moefRecordToRaw(record, base));
  ...
  await mapWithConcurrency(rawItems, DETAIL_FETCH_CONCURRENCY, async (item) => {
    const decorated = decorateRawItem(item);
    const normalizedItem = normalizeItem(decorated);
    if (!displayItemIds.has(normalizedItem.id)) return;
    if (!decorated.companyNoticeUrl) return;
  ...
  const ok = inventory.pages.length > 0;
  ...
  return {
    ...
    status: sourceStatus(base, {
      ...
      pagination: { ...inventory, records: undefined },
      failedUrlCount: inventory.issues.length,
      collectionScope: 'All ongoing public-institution notices in MOEF API; up to 40 pages, totalCount reconciled',
      firstDayCandidates,
      missedReviewNeeded: missedReview,
      message: ok
        ? `공공기관 API ${rawItems.length}/${inventory.expectedTotal ?? '?'}건, ${inventory.pages.length}페이지, 후보 ${normalized.length}건, 공식 공고 접속확인 ${companyChecked}건${inventory.complete ? '' : ', 목록 완전성 재점검 필요'}`
        : '공공기관 API 목록 조회 실패'
    })
  };
}
...
async function fetchMpmPublicJob() {
  ...
  const scans = await mapWithConcurrency(partitions, 4, async (params) => {
    ...
    return { partition: `${params.Instt_se}/${params.Pblanc_ty}`, ...inventory };
  });
  const list = new Map(scans.flatMap((scan) => scan.records).map((row) => [row.sourceId, row]));
  const activeRows = [];
  for (const row of list.values()) {
    row.source = base.id;
    row.sourceName = base.name;
    const deadline = parseDate(row.deadline);
    if (deadline && formatDate(deadline) < formatDate(NOW)) row.collectionDisposition = 'closed';
    else activeRows.push(row);
    COLLECTION_DISCOVERED.set(`${base.id}:${row.sourceId}`, row);
  }
  // Prioritize explicit school recruitment, then rotate untouched records on subsequent runs.
  ...
  const details = await mapWithConcurrency(activeRows.slice(0, 300), 4, async (row) => {
    try {
      ...
      return { ...row, qualification, description: qualification,
        companyNoticeUrl: cleanUrl(getXmlText(body, 'link01')) || cleanUrl(getXmlText(body, 'link02')),
        qualificationEvidenceIncomplete: false };
    } catch {
      detailFailed += 1;
      row.collectionDisposition = 'detail-failed';
      return null;
    }
  });
  const rawItems = details.filter(Boolean);
  const successScans = scans.reduce((n, scan) => n + scan.pages.length, 0);
  ...
  return {
    ...
    status: sourceStatus(base, {
      ...
      message: ok
        ? `나라일터 목록 ${list.size}건/${successScans}페이지, 진행 공고 상세 ${rawItems.length}건, 후보 ${normalized.length}건, 상세 실패 ${detailFailed}건·대기 ${pagination.detailDeferred}건`
        : '나라일터 공식 API 목록 조회 실패'
    })
  };
}
...
async function fetchJobAlioDetail(row) {
  ...
  return {
    ...
    qualification,
    qualificationEvidenceIncomplete: !qualification,
...
function jobAlioCriticalFallbackRawItem(row = {}, critical = {}) {
  ...
  return {
    ...
    education,
    career: '신입·공채 원문 확인',
    qualificationEvidenceIncomplete: true,
    employmentType,
...
async function fetchJobAlioRecruit() {
  ...
  for (const row of rowsByIdx.values()) COLLECTION_DISCOVERED.set(`job-alio-openapi:${row.idx}`, {
    ...row, source: 'job-alio-openapi', sourceId: String(row.idx),
    url: `https://job.alio.go.kr/recruitview.do?idx=${encodeURIComponent(row.idx)}`,
...
async function main() {
  ...
  COLLECTION_ASSESSED.clear();
  let previousCollectionAudit = {};
  try {
    previousCollectionAudit = JSON.parse(await fs.readFile(COLLECTION_AUDIT_FILE, 'utf8'));
    for (const row of previousCollectionAudit.records || []) {
      if (!['deferred', 'detail-failed', 'unexplained'].includes(row.disposition)) COLLECTION_PREVIOUS_KEYS.add(row.key);
    }
  } catch (error) {
    if (error.code !== 'ENOENT') console.warn('Previous collection audit could not be restored.');
  }
  try {
    const previous = JSON.parse(await fs.readFile(OUT_FILE, 'utf8'));
    for (const item of previous.qualificationReview || []) {
      if (item.deadline && new Date(item.deadline).getTime() < NOW.getTime() - 86400000) continue;
      QUALIFICATION_REVIEW.set(`${item.source}:${item.sourceId || item.id}`, item);
    }
  } catch (error) {
  ...
  const payload = {
    ...
    summary: {
      ...
      briefDirect,
      briefingReady,
      automatedBriefingReady: briefingAutomation.succeeded,
      automatedBriefingAttempted: briefingAutomation.attempted,
      automatedBriefingFailed: briefingAutomation.failed,
      firstDayCollected,
      lateDetected,
      missedReviewNeeded,
    ...
    qualificationReview: [...QUALIFICATION_REVIEW.values()],
    aiBriefing: briefingAutomation,
    collectionReview,
    secretReadiness,
    collectionPolicy: {
      ...
      briefingRule: '하루 3회 공식 원문과 구조화 규칙만으로 취업부 브리핑을 생성하며 외부 AI 실행 여부가 수집·게시를 차단하지 않는다.'
    },
    tracks: [
      {
        id: 'exam-formal',
