...
const LEARNING_APP_PATH = 'apps/sugar-salt/';
const LEARNING_APP_MIN_VERSION = '4.8.10';
...
async function validateProjectScope() {
  ...
  const isHomepageProject = homepageRemote || normalized.endsWith('/neojoin1-cyber-homepage')
    ...
    || normalized.includes('/meister-platform/portal-service-')
...
async function validateJobFetcherRules() {
  ...
  fail('fetcher.job-alio-bounded-notice-checks', fetcher.includes('JOB_ALIO_DETAIL_CONCURRENCY = 5') && fetcher.includes('JOB_ALIO_EMPLOYER_CHECK_LIMIT = 40') && fetcher.includes('selectJobAlioEmployerNoticeCheckCandidates') && fetcher.includes('[job-alio] detail progress='), '잡알리오 전수 상세 판정은 유지하면서 기관 페이지 접속 확인은 실제 학생 채널 후보로 제한하고 실행 진행 로그를 남깁니다.');
...
async function validateHomepage() {
  ...
  fail('home.feed-link-present', html.includes('href="vocational.html?v=20260802-navfix1"') && html.includes('특성화고 교육지원센터'), '대표 홈에서 특성화고 교육지원센터를 거쳐 채용정보에 접근하는 핵심 경로가 유지됩니다.');
  fail('home.core-content-links', html.includes('특성화고 교육지원센터') && html.includes('공직시험 연구소') && html.includes('디지털 프로젝트 스튜디오'), '대표 홈에서 세 핵심 사업 화면으로 이동하는 경로가 유지됩니다.');
  ...
  fail('home.law-link', html.includes('href="vocational.html?v=20260802-navfix1"') && html.includes('서식창고') && html.includes('상담실'), '대표 홈의 특성화고 교육지원센터 카드가 서식창고와 비공개 상담실 진입 방향을 안내합니다.');
  fail('home.no-legacy-ebook-link', !html.includes('https://gyo6--ebook.web.app'), '대표 홈페이지는 기존 전자책 서재 링크를 노출하지 않습니다.');
  fail('home.ebook-direction', !html.includes('href="ebooks.html"') && !html.includes('<strong>전자책</strong>'), '대표 홈은 현재 사업 구조에 맞춰 별도 전자책 바로가기를 노출하지 않습니다.');
  ...
  fail('home.vocational-two-axis', html.includes('href="vocational.html') && html.includes('채용정보 · 학습앱 · 서식창고 · 업무지침 · 상담실'), '채용정보·학습앱·서식창고·업무지침·상담실을 묶은 특성화고 교육지원센터 분기가 유지됩니다.');
...
async function validateCoreContentPages() {
  const brandPagePaths = [
    ...
    'apps.html', 'host.html', 'boards.html', 'contact.html', 'counseling-room.html', 'learning-app.html',
    'ebooks.html', 'forms.html', 'guides.html', 'jobs.html', 'miovera.html',
    'profile.html', 'resources.html', 'wheretoday.html', 'bubble-nova-star.html', 'web-production.html', 'parking/0mu1/index.html',
  ];
  ...
  const guides = await readText('guides.html');
  ...
  const bubbleNovaPage = await readText('bubble-nova-star.html');
  const webProductionPage = await readText('web-production.html');
  const learningAppTrial = await readText('apps/sugar-salt/index.html');
  const learningAppViewer = await readText('learning-app.html');
  ...
  const learningAppManifest = JSON.parse(await readText('apps/sugar-salt/manifest.json'));
  const learningAppVersion = JSON.parse(await readText('apps/sugar-salt/version.json'));
  const learningEntryMatch = learningAppTrial.match(/src="\/apps\/sugar-salt\/(assets\/index-[^"]+\.js)"/);
  const learningAppEntry = learningEntryMatch ? await readText('apps/sugar-salt/' + learningEntryMatch[1]) : '';
  const portalBoards = await readText('assets/portal-boards.js');
  const portalPopup = '';
  const boardEnhancements = await readText('assets/board-enhancements.css');
  const boardsPage = await readText('boards.html');

  const missingCorporateLogos = brandPages
    .filter(({ html }) => (!html.includes('brand/logo/badge.svg') && !html.includes('class="brand-mark brand-logo-svg"')) || !html.includes('class="brand-wordmark"') || !html.includes('gyo6.kr · 교육.한국'))
  ...
  fail('core.board-attachments-ui', boardsPage.includes('name=\"attachments\"') && portalBoards.includes('attachment/upload') && portalBoards.includes('attachment/delete') && portalBoards.includes('hydratePrivateImages') && portalBoards.includes('파일당 15MB'), '게시판이 이미지·문서 첨부, 권한별 미리보기·다운로드, 첨부 삭제를 제공합니다.');
  fail('core.no-partnership-recruitment', !homePage.includes('home-partnership-banner') && !homePage.includes('assets/portal-popup.js') && !exams.includes('exam-partnership-strip') && !portalBoards.includes('campaign:partnership') && !portalPopup, '제휴기관 모집 배너·팝업·게시판 강제 노출을 모두 중단합니다.');
  fail('core.public-service-entrypoints', homePage.includes('class="home-free-mock home-continuity-banner"') && homePage.includes('href="exams.html?v=20260802-navfix1"') && homePage.includes('기출 전수분석 · 내용·구조·형식') && exams.includes('class="branch-free-mock') && exams.includes('href="#engine"') && exams.includes('href="exam-service.html?service=civil"') && exams.includes('기출 전수분석 × T1·T2·T3') && examService.includes('https://0mu1.gyo6.kr/'), '메인은 교육 실천의 연속성과 공직시험 연구소를 연결하고, 연구소는 기출 전수분석·단계별 엔진 기준과 공무원관 진입 경로를 제공합니다.');
  ...
  fail('home.exam-lab-positioning', homePage.includes('<strong>공직시험 연구소</strong>') && homePage.includes('초·중등 교원임용 · 지방직 7급') && homePage.includes('기출 전수분석 · 내용·구조·형식') && homePage.includes('T1·T2·T3 단계 설계 · 적대적 검증'), '메인 두 번째 사업 카드가 공직시험 연구소의 공개 범위와 새 제작 기준을 정확히 안내합니다.');
  ...
  fail('core.vocational-priority-layout', vocational.includes('class="vr-main"') && vocational.includes('설탕과소금 JOB고 스킬캠퍼스') && vocational.includes('취업 준비 3년을') && vocational.includes('class="vr-app-showcase') && vocational.includes('class="vr-screen-link"') && vocational.includes('class="vr-entry-dock"') && vocational.indexOf('class="vr-screen-link"') < vocational.indexOf('class="vr-entry-dock"') && vocational.includes('공개체험 해보기') && vocational.includes('학교용 로그인') && vocational.includes('class="vr-jobs-quick" href="jobs.html"') && vocational.includes('class="vr-jobs"') && vocational.includes('특성화(마이스터)고') && vocational.includes('data-live-jobs') && vocational.includes('class="vr-hall-grid"') && ['교육부 직업공통능력관', 'NCS 직업기초능력관', '채용필기 심화관', '자기소개서관', '면접 스킬관', '인성검사 훈련관'].every((title) => vocational.includes(title)) && vocational.includes('class="vr-teacher-panel') && vocational.includes('class="vr-screen-grid"') && vocational.includes('class="vr-support-grid"') && vocational.indexOf('href="https://gyo6.kr/learning-app.html#trial-accounts"') < vocational.indexOf('class="vr-jobs"') && ['href="forms.html"', 'href="guides.html"', 'href="counseling-room.html"'].every((link) => vocational.includes(link)), '특성화고 교육지원센터는 앱 실제 화면과 동일한 중요도의 체험관·학교용 로그인을 첫 화면에 제공하고, 채용정보를 상단에서 바로 연결한 뒤 여섯 학습관·교사 수업·현장 지원 서비스를 안내합니다.');
  fail('core.frontier-ai-positioning', exams.includes('수능 450점 AI 시대') && exams.includes('AI가 수능을 다 맞힌 시대') && exams.includes('class="exam-frontier-proof"') && exams.includes('GPT‑6 Astra') && exams.includes('Claude Fable 5.1') && exams.includes('https://github.com/hehee9/2026-CSAT') && exams.includes('벤치마크는 기술 진전의 신호이며 본 서비스의 정답률 보증은 아닙니다.') && vocational.includes('수능 450점 AI 시대') && vocational.includes('취업교육은 검증까지') && vocational.includes('근거·반박·공개 게이트'), '두 교육 서비스는 수능 450점 AI 시대의 기술 진전을 출처·한계와 함께 제시하고, 한 모델의 답이 아닌 역할 분리 검증을 핵심 경쟁력으로 설명합니다.');
  fail('core.exam-service-links', exams.includes('href="exam-service.html?service=elementary"') && exams.includes('href="exam-service.html?service=secondary"') && exams.includes('href="exam-service.html?service=civil"') && examService.includes('https://chodeung.gyo6.kr/') && examService.includes('https://imyong.gyo6.kr/') && examService.includes('https://0mu1.gyo6.kr/') && examService.includes('id="service-frame"') && exams.includes('8개년 전수분석 · 현행 기준') && exams.includes('고난도 실전 모의고사 15회') && !exams.includes('49,900원') && exams.indexOf('class="branch-card-grid cols-3"') < exams.indexOf('class="exam-research-record"') && exams.includes('T1 · EVIDENCE LAYER') && exams.includes('T3 · ADVERSARIAL LAYER') && !exams.includes('suneung.gyo6.kr') && !exams.includes('수능시험'), '공직시험 연구소가 포털 상단 메뉴를 유지한 내부 화면에서 초등·중등 임용과 지방직 7급 시험을 먼저 제공하고, 금액 노출 없이 전수분석·단계별 엔진 기준을 이어서 안내합니다.');
  ...
  fail('core.learning-app-trial-link', vocational.includes('href="https://gyo6.kr/learning-app.html#trial-accounts"') && vocational.includes('href="https://app.gyo6.kr/"') && !vocational.includes('href="https://gyo6.kr/learning-app.html#trial-accounts" target=') && !vocational.includes('href="https://app.gyo6.kr/" target=') && vocational.includes('학생·교사 체험 · 현재 연결 점검 중') && vocational.includes('최신 v5 · 학교 계정·실제 기록') && learningAppViewer.includes('class="service-viewer-body learning-trial-page"') && learningAppViewer.includes('id="trial-accounts"') && ['data-trial="student"', 'data-trial="teacher"', '?trial=${role}&trial_nonce=${Date.now()}'].every((marker) => learningAppViewer.includes(marker)) && learningAppViewer.includes('공개체험 연결 점검 중') && !['demo.student@sugarsalt.kr', 'demo.teacher@sugarsalt.kr', 'demo.admin@sugarsalt.kr', 'sugarsalt2026'].some((secret) => learningAppViewer.includes(secret)) && learningAppViewer.includes('id="service-frame"') && learningAppViewer.includes('src="' + LEARNING_APP_PATH + '"') && learningAppManifest.start_url === './?entry=member' && versionAtLeast(learningAppVersion.version, LEARNING_APP_MIN_VERSION) && learningAppVersion.supabaseResponseCache === false && /entry.{0,24}member/.test(learningAppEntry), '특성화고 교육지원센터는 동일 창에서 무제한·읽기 전용 체험관과 앱 직접 인증 방식의 정식사용자 로그인을 분리해 제공합니다.');
...
fail('core.google-play-status-cards', (appsPage.match(/class="google-play-mark"/g) ?? []).length === 4 && (appsPage.match(/Google Play 등록 절차 진행 중 · 비공개 테스트/g) ?? []).length === 3 && appsPage.includes('<strong>Fast Profile</strong>') && appsPage.includes('<strong>미오베라 모험동화</strong>') && appsPage.includes('<strong>오늘어디가</strong>'), 'Fast Profile·미오베라·오늘어디가는 Google Play 비공개 테스트 등록 진행 상태를 표시하고 버블 노바 스타를 포함한 네 앱에 스토어 아이콘을 제공합니다.');
  fail('core.web-production-page', appsPage.includes('href="web-production.html"') && webProductionPage.includes('정보구조부터') && webProductionPage.includes('모든 화면에서') && webProductionPage.includes('공개 이후까지') && webProductionPage.includes('https://ktedu.or.kr/'), '홈페이지 제작·운영이 내부 사례 소개에서 기획·반응형 제작·운영 지원과 실제 사례를 안내합니다.');
  fail('core.learning-app-trial-build', learningAppTrial.includes('manifest.json') && learningAppTrial.includes('registerSW.js') && learningAppTrial.includes('설탕과소금 - 달콤한 취업 성공을 위한 짭짤한 실력 준비'), '설탕과소금 앱 경로가 전달받은 로그인형 PWA 빌드와 설치 구성을 그대로 제공합니다.');
  fail('core.learning-app-trial-worker-cleanup', learningAppRegister.includes("navigator.serviceWorker.register('/apps/sugar-salt/sw.js', { scope: '/apps/sugar-salt/' })") && learningAppWorker.includes('precacheAndRoute') && learningAppWorker.includes('cleanupOutdatedCaches'), '로그인형 PWA가 해당 앱 절대 경로에서만 서비스워커를 등록하고 새 번들의 사전 캐시와 구 캐시 정리를 수행합니다.');
}
...
async function validateLive(localFeed) {
  ...
  try {
    ...
    fail('live.learning-app.same-origin-frame', viewer.text.includes('src="' + LEARNING_APP_PATH + '"'), '라이브 체험관 iframe이 홈페이지 내부 앱 릴리스를 불러옵니다.');

    const versionResponse = await fetchText(LIVE_HOME + LEARNING_APP_PATH + 'version.json?' + cacheBust);
    const release = JSON.parse(versionResponse.text);
    fail('live.learning-app.release-version', versionResponse.ok && versionAtLeast(release.version, LEARNING_APP_MIN_VERSION), '홈페이지 내부 학습앱이 ' + LEARNING_APP_MIN_VERSION + ' 이상입니다.', release.version || 'unknown');
    fail('live.learning-app.no-auth-response-cache', release.supabaseResponseCache === false, '학습앱의 Supabase 인증·데이터 응답 캐시가 비활성화되어 있습니다.');
    const memberResponse = await fetchText('https://app.gyo6.kr/?' + cacheBust);
    fail('live.learning-app.member-entry-status', memberResponse.ok && memberResponse.text.includes('/assets/'), '학교용 진입 URL이 앱 엔트리를 HTTP 200으로 제공합니다. 로그인 성공 여부는 별도 검증합니다.', 'status ' + memberResponse.status);
  } catch (error) {
    fail('live.learning-app.release-check', false, '홈페이지 내부 학습앱 릴리스 확인에 실패했습니다.', error.message);
  }
}
