...
const deploymentConfig = read("supabase/config.toml");
const normalizeLineEndings = (value) => value.replace(/\r\n/g, "\n");
const headers = read("review/_headers");
const managerHtml = read("review/manage.html");
const managerCss = read("review/manage.css");
...
check("content.source-tables", js.includes("REVIEW_TABLE_V1") && js.includes("renderTableGroup") && css.includes("review-data-table") && css.includes("review-group-actions"), "원본 표를 셀별 문장으로 흩뜨리지 않고 전문 검수용 표로 복원합니다.");
check("content.grouped-confirmation", js.includes("TEXT_CONFIRM_CHUNK_SIZE = 4") && js.includes("renderTextGroup") && js.includes("review-confirm-chunk") && !js.includes("data-check-block=\"${escapeHtml(block.id)}\""), "일반 본문은 같은 소제목의 연속 문단을 네 문단 단위로 묶어 확인 부담을 줄입니다.");
check("content.table-progress", js.includes("표 전체 확인") && js.includes("data-check-blocks") && js.includes("toggleBlocks") && js.includes("review-table-cell"), "표 전체 확인과 셀별 검수 의견이 기존 진행기록·보고서에 연결됩니다.");
...
check("auth.otp-workroom-return", edge.includes("검수 워크룸으로 돌아가기") && edge.includes('href="${emailHtml(REVIEW_APP_URL)}"') && edge.includes("접속 주소:"), "인증번호 메일만 다시 열어도 검수 워크룸 링크와 실제 접속 주소를 확인할 수 있습니다.");
...
check("deployment.review-asset-version", html.includes("review.css?v=20260815-4") && html.includes("review.js?v=20260815-8") && managerHtml.includes("manage.js?v=20260815-10") && managerHtml.includes("manage.css?v=20260815-10"), "일반 새로고침만으로도 전문위원·관리자 화면이 최신 검수 코드를 받습니다.");
check("review.live-presence", js.includes('api("heartbeat"') && edge.includes('action === "heartbeat"') && edge.includes('from("review_presence")') && managerHtml.includes('id="summary-online"') && managerJs.includes('["online","away"].includes(row.presenceStatus)') && managerJs.includes("현재 검수 화면 사용 중") && managerJs.includes("다른 화면 확인 중"), "워크룸의 최신 접속상태를 별도 최신값으로 갱신하고 관제 화면에 검수 화면 사용·다른 화면 확인·화면 잠금·연결 없음을 오해 없이 구분합니다.");
check("security.away-screen-lock", html.includes('id="screen-lock"') && html.includes('id="pin-setup-dialog"') && css.includes(".screen-lock[hidden]") && js.includes("SCREEN_LOCK_IDLE_MS") && js.includes("PBKDF2") && js.includes('sendPresence("locked")'), "14일 인증을 유지하면서 6자리 기기 화면잠금과 자리 비움 자동 가림을 제공합니다.");
check("deployment.public-otp-route", deploymentConfig.includes("[functions.review-content]") && deploymentConfig.includes("verify_jwt = false") && edge.includes('if (action === "requestOtp")') && edge.includes("const token = bearer(request)"), "로그인 전 인증번호 요청은 함수 내부의 출처·대상·속도 제한을 거치고 나머지 기능은 사용자 토큰을 검증합니다.");
...
check("review.annotation-filter-clarity", ["전체 기록", "수정 권고", "전문 참고의견", "형광펜 표시", "현재 자료에 저장된 기록을 종류별로 골라 봅니다."].every((value) => html.includes(value)) && js.includes('item.setAttribute("aria-pressed", String(selected))'), "문장별 검수기록 필터의 역할을 명확히 구분하고 선택 상태를 보조기술에도 전달합니다.");
...
check("report.large-volume-stability", edge.includes("INTEGRITY_DETAIL_LIMIT = 200") && edge.includes("uncheckedOmittedCount") && edge.includes("자료별 미확인 범위") && js.includes("integrityByDocument") && js.includes("나머지 ${omittedUnchecked}건"), "수천 문단 원고도 전체 집계는 보존하면서 보고서 응답을 제한하여 브라우저에서 안정적으로 미리봅니다.");
check("report.unchecked-source-minimization", !edge.includes("excerpt: cleanText(block.body") && !edge.includes("reportValue(item.excerpt)") && !js.includes('escapeHtml(item.excerpt || "")'), "미확인 문단은 위치·분량만 보고하고 원문 내용은 보고서 응답과 미리보기에 재노출하지 않습니다.");
check("report.reviewer-export-boundary", js.includes("canExportReviewReport") && js.includes('state.mode === "manager-preview"') && js.includes("보고서 파일 저장과 인쇄는 회사 관리자 확인 화면에서 제공합니다"), "전문위원은 화면에서 의견을 확인·보완하고 회사 관리자만 공식 보고서를 저장·인쇄합니다.");
...
check("manager.activity-audit-accuracy", edge.includes("REVIEWER_ACTIVITY_EVENT_TYPES") && edge.includes("reviewerActivityEvents") && edge.includes("lastActivityLabel") && managerJs.includes("row.lastActivityLabel"), "내부 원고 재구성 작업을 전문위원 활동에서 제외하고 인증·로그인·열람·확인 활동의 종류와 시각을 표시합니다.");
...
check("manager.termination-audit", managerHtml.includes('id="termination-form"') && managerHtml.includes('id="termination-history-dialog"') && managerJs.includes('review_assignment_terminations') === false && edge.includes('from("review_assignment_terminations")') && edge.includes('reason_detail') && edge.includes('terminated_by'), "해촉 사유·경과·처리자·처리시각·접근 차단 상태를 별도 감사기록으로 보존합니다.");
check("manager.termination-directory", managerHtml.includes('data-directory-filter="terminated"') && managerJs.includes("openTerminationHistory") && managerJs.includes("활동 기록 보존"), "활동 중 위원과 해촉·비활성 위원을 분리 조회하고 기존 활동량과 해촉 기록을 확인합니다.");
check("manager.termination-requires-reason", managerJs.includes('id="termination-reason"') === false && managerHtml.includes('id="termination-reason"') && edge.includes('reasonDetail.length < 10') && edge.includes('allowedReasonCodes'), "위촉 종료 전 사유와 사실 경과를 필수로 기록하여 임의 종료를 방지합니다.");
check("manager.termination-compensation", edge.includes('기존 상태를 복구했습니다') && edge.includes('review_assignment_terminations").delete()') && edge.includes('assignment.status === "revoked"'), "해촉 이력·계정 차단·과제 상태 저장 중 실패하면 중간 변경을 복구하고 중복 해촉을 차단합니다.");
...
check("manager.communication-repair", managerHtml.includes('id="repair-start-notice"') && managerHtml.includes('id="communication-action-guide"') && managerHtml.includes("실제 메일 발송·기록 복구") && managerCss.includes("communication-audit-dialog[open]") && managerJs.includes('data-notice-repair-id') && managerJs.includes("managerResendStartNotice") && edge.includes('action === "managerResendStartNotice"') && edge.includes("assignment-start-v2-repair") && edge.includes("assignment_notice_record_repaired"), "후속 검수 기록은 있지만 개시 안내 기록만 누락된 예외를 관제 목록에서 바로 확인하고, 고정된 다음 단계 버튼으로 중복 없이 복구합니다.");
...
check("manager.latest-access-tracking", managerJs.includes("lastWorkroomEnteredAt") && managerJs.includes("lastDocumentOpenedAt") && managerJs.includes("워크룸 최초·최근 접속") && managerJs.includes("원고 최초·최근 열람") && edge.includes("lastWorkroomEnteredAt") && edge.includes("lastDocumentOpenedAt"), "관제센터에서 최초 기록뿐 아니라 최근 로그인·워크룸 접속·원고 열람까지 함께 확인합니다.");
check("manager.client-version-tracking", js.includes("REVIEW_CLIENT_VERSION") && js.includes("clientVersion: REVIEW_CLIENT_VERSION") && edge.includes("latestClientVersion") && managerJs.includes("latestClientVersion"), "오래 열린 이전 화면과 최신 검수 화면을 서버 접속 기록에서 구분합니다.");
check("manager.stage-legend", managerHtml.includes("수행 완료") && managerHtml.includes("다음 단계") && managerHtml.includes("기록 누락") && managerHtml.includes("적색=누락") && managerJs.includes('index<lastCompleted?"missing"'), "채운 원은 수행 완료, 황동 테두리는 다음 차례, 적색 느낌표는 후속 활동 대비 기록 누락임을 명확히 설명합니다.");
check("manager.completed-is-not-terminated", managerJs.includes('rows.every(row=>row.status==="revoked")') && !managerJs.includes('row.status==="revoked"||row.status==="accepted"'), "정상 완료 과제를 해촉으로 오분류하지 않습니다.");
...
check("security.selection-copy-boundary", js.includes("selectionWithinProtectedSurface") && js.includes('protectedReviewSurface = ".review-paper, .report-preview-dialog, .submission-integrity-dialog"') && js.includes('addEventListener("cut"') && js.includes('addEventListener("dragstart"'), "본문 선택은 검수 도구에 사용하되 선택 범위 기준으로 복사·잘라내기·텍스트 끌어내기를 차단합니다.");
