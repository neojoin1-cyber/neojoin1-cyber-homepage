...
check("manager.email-event-fallback", edge.includes('startNotificationStatus === "sent" ? startEvent?.occurred_at') && edge.includes('notificationRecordStatus="event_fallback"') && edge.includes("notificationSentAt,notificationId:notification.id"), "메일 발송 시각 열 저장이 누락돼도 발송 성공 이벤트를 소급 인정하고 이후 발송 기록을 이중 보존합니다.");
